...
def fetch_supabase_youtube_tracks():
    """Récupère tous les morceaux avec liens YouTube depuis Supabase."""
            ...
            for item in data:
                for key in ["audio_url", "soundtrack_url", "music_url", "youtube_url"]:
                    ...
                    if raw_url and ("youtube.com" in raw_url or "youtu.be" in raw_url) and raw_url not in seen_ids:
                        ...
                        found_tracks.append({
                            "title": item.get("title") or item.get("scene_title") or "Proposition Supabase",
                            "artist": item.get("creator_name") or "",
                            "url": raw_url,
    ...
    # 3. Si aucun morceau en base, proposer la liste par défaut
    if not found_tracks:
        print("\n[INFO] Aucun morceau YouTube personnalise trouve dans la base Supabase distante.")
...
def download_and_extract_track(track_info: dict, output_dir: Path, temp_dir: Path):
    """Télécharge et sépare un morceau spécifique."""
    url = track_info["url"]
...
def main():
    ...
    tracks = fetch_supabase_youtube_tracks()
    print(f"\n[TOTAL] {len(tracks)} lien(s) YouTube a traiter :")
    for i, t in enumerate(tracks, 1):
        print(f"  {i}. {t['title']} ({t['url']})")

    print("\n" + "=" * 65)
    confirm = input("Lancer la separation automatique de tous ces morceaux ? (O/n) : ").strip().lower()
    ...
    try:
        for idx, track in enumerate(tracks, 1):
            print(f"\n[{idx}/{len(tracks)}] Traitement de : {track['title']}")
