...
echo  1. L'application Web (Node.js & dependances npm)
...
if %errorlevel% neq 0 (
    ...
    echo ⚠️ N'oubliez pas de redemarrer votre terminal apres installation.
    pause
    exit /b 1
)
echo ✅ Node.js detecte. Installation des modules npm en cours...
call npm install
if %errorlevel% neq 0 (
...
echo ✅ Dependances Web installees avec succes !
echo.
...
if %errorlevel% neq 0 (
    ...
    echo ⚠️ IMPORTANT : Cochez bien la case "Add python.exe to PATH" lors de l'installation !
    pause
    exit /b 1
)
echo ✅ Python detecte.
...
echo ✅ Paquets Python et IA installes avec succes !
echo.

:: 4. Configuration automatique de FFmpeg
echo ⚙️ 4/4. Configuration de FFmpeg et verification finale...
python -c "import imageio_ffmpeg, os, shutil; exe = imageio_ffmpeg.get_ffmpeg_exe(); target = os.path.join(os.path.dirname(exe), 'ffmpeg.exe'); shutil.copyfile(exe, target) if not os.path.exists(target) else None; print('✅ FFmpeg configure avec succes !')"
